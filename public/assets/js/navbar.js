...
function appuiIconeBurger() {
    ...
    if (oPos === "-150px") {
        ...
        // On réactive le clic pour les liens du menu
        oMenu.style['pointer-events'] = "initial";
        /*for (let i = 0; i < oLienMenu.length; i++) {
            let elem = oLienMenu[i];
...
function closeMenuBurger() {
    ...
    setTimeout(closeMenuBurgerSuite, 300); // Attend la fin de la transition avant de virer la class
    // ============== //
...
function closeMenuBurgerSuite() {
    ...
    oBody2.style.visibility = "hidden"; // On masque la div body2 pour ré-eclaircir l'écran
}
...
// let oLienMenu = document.getElementById("menuId").getElementsByTagName("a");
...
document.addEventListener('click', function (event) {
    ...
    let onCliqueDansVideMenu = oMenu.contains(event.target);
